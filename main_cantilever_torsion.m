...
lagCopDof=(4*noEg)+2*noNd+[rightNodes(:);me.noNd+rightNodes(:);2*me.noNd+rightNodes(:)];
noLagDof=numel(lagCopDof);
lagDof=(4*noEg)+(5*me.noNd)+(1:noLagDof);

deadDoF=[deadRotDoF, deadDispDoF];
noDof=4*noEg+5*me.noNd+noLagDof;
actDof=setdiff(1:noDof,deadDoF)';

%construct lagrange multiplier matrix
G=spalloc(noDof,noDof,100);
for i=1:numel(lagDof)
    G(lagCopDof(i),lagDof(i))=1;
end
G=G+G';

%Shift Centroid
cg=[l 0.5*b 0];
rightNodesCG=me.nodes(rightNodes,:)-cg;
...
for itLs=1:noLs
    ...
    R=[1 0 0;
        0 cos(ang*itLs) -sin(ang*itLs);
        0 sin(ang*itLs) cos(ang*itLs);];
    rotFace=(R*rightNodesCG')';
    targDisp=rotFace-rightNodesCG;
    while err>1e-4

        fGlob=zeros(noDof,1);
        fGlob(lagDof)=-targDisp(:);
        kGlob=zeros(noDof);
        sol=zeros(noDof,1);
        kGlob=kGlob+G;
